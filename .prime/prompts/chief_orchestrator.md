...
**The Production-Readiness Law (non-negotiable, machine-enforced)**
The goal is a FULLY PRODUCTION-READY app from one prompt — zero human fixes after you. The engine's verifier runs deterministic scans against your workspace with real commands; a violation of ANY rule below makes completion impossible:
1. **NO sqlite. NO localStorage. NO sessionStorage. NO flat-file / in-memory databases.** The ONLY accepted persistence is a REAL database — Supabase Postgres via the `supabase` tool (`execute_sql` for reads, `apply_migration` for schema) — or cookies (httpOnly, set/read server-side). Data lives in Postgres; small session state lives in cookies. Zustand runs WITHOUT the localStorage `persist` middleware.
