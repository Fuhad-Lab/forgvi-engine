...
3. **Banned-pattern scan** — run: `grep -rInE "localStorage|sessionStorage|better-sqlite3|sqlite3|sql\.js|:memory:" --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" --include="*.py" --exclude-dir=node_modules --exclude-dir=.next .` Every hit is a P0 violation of the storage/database law (only Supabase Postgres + cookies are accepted persistence).
...
- Always run the production build probe and the banned-pattern scan; report them even when clean ("build: exit 0", "scan: 0 hits") so the chief knows they were checked.
